...
\begin{lem}\llabel{lem:rip-step2-1}\label{lem:SS-induct}
...
Apply Lemma~\ref{lem:rip-step2-1} with $\si=\be$. Basically, we're going to inductively construct a subset $\sigma = \beta$ of $\{1, \cdots, m\}$ onto which to project, so that we can control the $l_1$ norm of the subset $\tau$ by the $l_2$ norm on a slightly greater set $\theta$ via Lemma~\ref{lem:rip-step2-1}. 

We find $\tau\subeq \be$ with $|\tau|\ge \pa{1-\rc{2^t}}|\be|$ such that 
\[
...
Denote $\ep=\fc{|\be|}{4|\tau|}$. By Lemma~\ref{lem:projbound}, there exists $\si\subeq \tau$, $|\si|\ge (1-\ep)|\tau|$ such that 
\bal
...
Now, we have basically finished making use of Lemma~\ref{lem:rip-step2-1} in the proof of Lemma~\ref{lem:rip-step2-2}. It remains to use Lemma~\ref{lem:rip-step2-2} to prove the full second step in our proof of the Restricted Invertibility Principle. 
...
$
%use lemma
%add more and more pieces.
%set up so can get what want, ...
%\te decreasing
As we induct, we are essentially building up more $\sigma_u$ to eventually produce the invertible subset over which we will project. 
Note that the size of the $\theta$ set is decreasing as we proceed inductively. 
\beq{eq:rip-s2-1}
...
\] 
since $A^{-1}A = I$ and projecting a subset onto its containing set is just the subset itself. 
%a subset of $J_\si$. Apply $A$ to it.
...
\end{proof}
%It remains to prove Lemma~\ref{lem:rip-step2-1}. Kiran: (see above)
...
%subset of proportional size, first bite gets the constrant fraction. To get to $1-\ep$. Each time, get half of what remains. it would be nice. get very close to rank.  % kiran: i think he said this was naive?
%aggressive
