...
For $k, m, n \in \N$, $A: \R^m \to \R^n$, $\rank(A) > k$. Let $\omega = \{1, \ldots, m\}$ with $|w| = \txtn{rank}(A)$ such that $\{Ae_j\}_{j \in \omega}$ are linearly independent. Denote for every $j \in \Omega$ 
\[
...
&\le \min_{k<r<\rank(A)}\sfc{\rank(A)}{\rank(A)-r} \max_{j\in \om} \sqrt{\ve{\Proj_{F_j} Ae_j}}\\
