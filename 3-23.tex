...
\bal
%\fixme{FIX}
\ve{H(F(x))-Jx}_{\ell_\iy}
%&\le \ve{H(F(y)) - G(F(y))}_{\ell_{\iy}} +\ve{G(F(y))-G(f(y))}_{\ell_\iy}\\
&\le nD\de + D \de+\de\\
&\lesssim nD\de.
\end{align*}
For all $x\in \rc2 B_X$, $\ve{H\circ F(x)-Jx}\le Cn D\de$. Define $g(x)=H\circ F(x)-Jx$. Then $\ve{g}_{L^{\iy}(\rc 2B_X)}\le CnD\de$. 
